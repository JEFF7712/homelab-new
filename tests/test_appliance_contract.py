...
class ApplianceContractTests(unittest.TestCase):
    ...
    def test_services_and_persistence_are_declarative(self) -> None:
        ...
        self.assertIn('domain = "registry.rupan.dev"', role)
        self.assertIn('answer = "10.0.30.20"', role)
        self.assertIn("enabled = true", role)
        ...
        self.assertIn("services.mosquitto", role)
        self.assertIn('address = "10.0.30.10"', role)
        self.assertIn("port = 1883", role)
        self.assertIn('"home-assistant"', role)
        self.assertIn("zigbee2mqtt =", role)
        self.assertIn("mosquitto-home-assistant-password", role)
        self.assertIn("mosquitto-zigbee2mqtt-password", role)
        self.assertIn("ConditionPathExists", role)
