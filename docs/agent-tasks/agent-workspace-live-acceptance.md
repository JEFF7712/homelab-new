...
| --- | --- | --- |
| Two simultaneous guests | Cross-workspace packet and login denial; bounded CPU, disk, and network pressure; etcd, authenticated PostgreSQL, and Home Assistant health before, during, and after load | Not run |
...
Do not add enabled inventory entries or distribute ingress credentials until every enrollment gate above is accepted against the same deployed revision or explicitly revalidated after a newer deployment.
