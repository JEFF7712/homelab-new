...
| --- | --- | --- |
| Two simultaneous guests | Two 2-vCPU, 2 GiB guests ran concurrently for 60 seconds. Each used two CPU workers, wrote 1 GiB to its data disk, downloaded from operator-owned `photos.rupan.dev`, and failed to connect to the peer's SSH port. The slice used 121.57 CPU seconds during the 63.55-second observation window under its 2 CPU quota, peaked at 4,453,249,024 bytes memory, and received 759,887,130 bytes. Eleven in-load samples retained 3/3 Ready nodes, 3 etcd voters, successful PostgreSQL `SELECT 1` in at most 0.317 seconds, HA HTTP 200 in at most 0.024 seconds, and no node pressure conditions. Raw evidence is [versioned with this record](evidence/agent-workspace-two-guest-2026-09-13.json). | Accepted at deployed revision `914629e8ffd1df8516c9661c6f12476fcb288e53` |
