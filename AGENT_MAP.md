...
The stable command surface is listed by `python -m scripts.agent --help` and the `justfile`. In this first work package, only `context` is implemented. Other commands fail with an explicit unavailable status until their owning work package delivers them.
