...
**Version:** 0.6.2  
**Status:** Audit-cleared for preliminary CAD and fit coupons; key T1 opening/rail-phase measurements now captured; production approval remains prototype-gated  
**Date:** 2026-09-13
...
```text
R-01: lid-support plane → topmost rack-hole center = 28.8 mm
R-02: top hole 1 → hole 2 = 15.9 mm
R-03: top hole 2 → hole 3 = 15.9 mm
```

The 15.9 mm spacings closely match the conventional 15.875 mm intra-U spacing.

One optional confirmation measurement remains useful before final rail generation:

```text
R-04: hole 3 → hole 4
```

A value near 12.7 mm would confirm the conventional repeating 15.875 / 15.875 / 12.7 mm pattern.

Preliminary CAD may proceed using the measured phase plus the conventional standard as a provisional model, but final production rail geometry should either confirm R-04 physically or reproduce the complete stock rail pattern from a direct measurement/CAD reference.
...
1. rack-hole phase, extreme-U clearance, and transverse clear opening,
...
`P` means **full-thread** protrusion through the locking element. Any chamfered or otherwise incomplete thread at the rod tip must be accounted for separately as `T`.

**Do not subtract exterior clearance from the rod-length equation.**

After final tightening, creep/compression dwell, and re-torque/re-seat procedure if applicable, verify exterior clearance at **both ends independently**. Do not assume the rod remains perfectly centered within the assembly.

Using the earlier simplified geometry only as an **idealized square-cut example** with `T = 0`:
...
L = 339.6 + 2 × (1.2 + 7.8 + 1.6 + 0.0)
  = 360.8 mm
```

A real cut rod with chamfered/incomplete end threads will require a measured nonzero `T` allowance.

The idealized example leaves approximately 0.2 mm exterior clearance per end in an 8.0 mm pocket:

```text
An 8.0 mm pocket is no longer viable with the measured nut:

1.2 + 7.8 + 1.6 = 10.6 mm

before adding chamfer/incomplete-thread allowance and exterior clearance.
```
