...
**Version:** 0.5  
**Status:** Post-audit structural revision; architecture retained but load paths, usable-8U geometry, and interference constraints remain to be validated  
**Date:** 2026-09-13
...
### Required physical rail-phase measurement

Before production CAD is frozen, measure on the physical T1:

```text
R-01: lid-support plane → center of topmost rack hole
R-02: center-to-center spacing of the top three existing rack holes
R-03: lid-support plane → center of second-highest rack hole
R-04: lid-support plane → center of third-highest rack hole
```

These measurements establish the actual hole phase and whether the T1 uses a conventional 3-hole-per-U pattern or another pattern.
...
The previous shorthand of two 177.8 mm free-standing column bodies plus two separate 12 mm frames is **not permitted**, because that would overrun the 355.6 mm envelope.
...
The front/rear rack face is the primary datum. Column material grows away from that face.

### Exact rail flushness

The extension rack faces shall remain coplanar with the corresponding stock T1 rack faces:
...
A thin pocket floor spanning the hollow 22 × 22 mm cavity is not acceptable.
...
- leaves no unsupported 4 mm membrane beneath the washer,
...
The male/female spigot concept remains, but must now satisfy explicit interference constraints.
...
- Front and rear rails shall remain parallel and coplanar with their stock counterparts.
...
The earlier 5-8 mm generic "insert region" is replaced with explicit dimensional requirements.
...
This maximum shall be documented in the assembly spec and, if practical, enforced geometrically with a blind boss / stop.
...
1. rack-hole phase and extreme-U clearance,
...
The assembly procedure shall be based on a tested preload / compression target.

Preferred prototype method:

1. Assemble one complete column with final washers, nuts, printed end blocks, and splice.
2. Tighten gradually using a repeatable torque tool or controlled nut-turn method.
3. Measure column shortening and inspect end-block deformation.
4. Record the torque / nut-turn value at the first condition that fully seats all bearing shoulders without visible PETG distress.
5. Hold the prototype under representative compressive load at expected operating temperature.
6. Re-measure after:
   - 1 hour,
   - 24 hours,
   - 7 days.
7. Establish a conservative assembly setting from the observed relaxation.

### Initial preload ceiling

No numeric production preload is specified in v0.5.

A final preload requirement may only be added after the prototype test above.
...
## 35. M5 Tie-Rod Length

### Status

The previous 355 mm cut length is **provisional**, not production-locked.

The purchased rods are:
...
### Why 355 mm remains only a starting point

Using the previous simplified assumptions:

```text
overall lid-support-plane spacing = 355.6 mm
washer = 1.2 mm
nyloc ≈ 5.0 mm
desired visible thread beyond nut ≈ 1.6 mm
```

produces a nominal rod length near 355.2 mm.

However, that result is sensitive to:

- actual assembled washer-seat spacing,
- actual purchased nut height,
- printed frame compression,
- PETG creep,
- final end-block geometry,
- required thread engagement through the nylon locking element,
- rod-end chamfer length,
- desired exterior clearance,
- manufacturing tolerance.

Therefore:

```text
355 mm = design estimate only
```

### Final derivation method

After prototype parts are printed, measure the actual assembled distance between the two washer bearing seats for one complete column.

Then calculate rod length from:

```text
measured washer-seat spacing
+ actual washer/nut stack allowance at both ends
+ required full-thread protrusion through each nyloc
- explicit exterior recess clearance at both ends
```

Final rod tolerance shall be derived from those measured quantities rather than preassigned as ±0.5 mm.

### Purchase decision

The purchased 400 mm rods remain appropriate because they leave sufficient trimming allowance regardless of modest design changes.
...
Final cut length and tolerance: TBD from measured assembled washer-seat spacing
```
