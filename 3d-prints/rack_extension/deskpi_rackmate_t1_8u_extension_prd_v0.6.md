...
**Version:** 0.6  
**Status:** Post-second-audit revision; architecture retained, with remaining geometry and prototype gates made explicit  
**Date:** 2026-09-13
...
Until that measurement is supplied, use the earlier provisional reference of approximately:

```text
222.25 mm clear opening
```

only as a design check, not as a final dimension.

With a 281 mm overall width, two fully inward-projecting 30 mm columns would leave only:

```text
281 - 2 × 30 = 221 mm
```

which is potentially too narrow.

Therefore, `30 × 30 mm` remains only a nominal gross envelope. The transverse section may need:

- reduced inward width,
- asymmetric geometry,
...
W = actual washer thickness
N = actual nut height
P = required full-thread protrusion beyond each nut
C = required exterior clearance between rod end and pocket exterior
```

Required rod length is:

```text
L = S + 2 × (W + N + P)
```

Exterior pocket depth is a separate constraint:

```text
pocket depth ≥ W + N + P + C
```

**Do not subtract exterior clearance from the rod-length equation.**

Using the earlier simplified geometry only as an example:
...
L = 339.6 + 2 × (1.2 + 5.0 + 1.6)
  = 355.2 mm
```

That example already leaves approximately 0.2 mm exterior clearance per end in an 8.0 mm pocket:

```text
8.0 - (1.2 + 5.0 + 1.6) = 0.2 mm
```
...
7. calculate final rod length from the equation above.
